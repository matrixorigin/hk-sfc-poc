...
log ""
log "========== Step 7: 创建索引 =========="
run_sql "CREATE INDEX IF NOT EXISTS idx_hsi_closing_date ON ms_t_stk_hsi(CLOSING, trade_date);"
run_sql "CREATE INDEX IF NOT EXISTS idx_sis_stk_date ON ms_t_stk_sis(SISTKC, trade_date);"
run_sql "CREATE INDEX IF NOT EXISTS idx_cap_date_stk ON ms_v_stock_capital(ref_date, STKCD);"
run_sql "CREATE INDEX IF NOT EXISTS idx_ind_stk_date ON ds_t_int_hsicl_dtl(STOCK_CODE, MODIFIED_DATE);"
run_sql "CREATE INDEX IF NOT EXISTS idx_news_date_sec ON sehknews(securitycode, timestamp);"
run_sql "CREATE INDEX IF NOT EXISTS idx_pl_stk_yr ON profit_loss(stock_code, fin_yr);"
run_sql "CREATE INDEX IF NOT EXISTS idx_hsi_daily_date ON ms_v_stk_hsi_daily(trade_date);"
log "索引创建完成"
