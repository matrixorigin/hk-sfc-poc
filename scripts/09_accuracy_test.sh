...
#     rows=N        精确行数
#     rows~N        行数 ±20%
#     contains=a,b  结果数据必须包含这些值
#     sql_contains=a,b  SQL文本必须包含这些关键字
#     excludes=a,b  结果数据不能包含这些值
# ============================================================
assert_case() {
...
" 2>/dev/null)
  ...
  for check in $checks; do
    ...
    # sql_contains=a,b  SQL 文本必须包含（用于检查过滤条件关键字）
    elif [[ "$check" == sql_contains=* ]]; then
      local vals="${check#sql_contains=}"
      local IFS=','
      for val in $vals; do
        if ! echo "$llm_sql" | grep -q "$val"; then
          echo "  ❌ $label — SQL missing keyword '$val' ($details)"
          return 1
        fi
      done

    # excludes=a,b  结果数据不能包含
